...
public:
    ...
    bool search(T const& val) const noexcept;
    bool compare(AVLTree const& other) const;

    bool insertRecursive(T const& val);
    bool removeRecursive(T const& val);
    bool searchRecursive(T const& val) const noexcept;
    bool compareRecursive(AVLTree const& other) const;
    ...
    [[nodiscard]] int8_t   height() const noexcept;
