...
struct binaryTree 
{
    ...
    */
    ...
    static bool isValidAVL(binaryTree* node) {
        if(!isValidBSTRecursive(node)) {
            return false;
        }
        return isValidAVL_InternalRecursive(node);
    }
    ...
    static bool insertIterative(binaryTree* node, uint32_t val, binaryTree** out) {
...
struct AVLTree {
    ...
    bool insert(uint32_t val) {
        ...
        bool        status       = binaryTree::insertIterative(m_root, val, &newRootMaybe);
