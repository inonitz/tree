...
template<typename T> void GenericRecursiveAVLTreeTest<T>::SetUp() {
...
template<typename T> void GenericRecursiveAVLTreeTest<T>::TearDown() {
...
template<typename T>
void GenericRecursiveAVLTreeTest<T>::printTreeToMassiveBuf(void const* root, int space) {
...
template<typename T> static std::array<T, 7> generateDataForRebalanceTest() {
    return { 
        __scast(T, 50), 
        __scast(T, 25), 
        __scast(T, 75), 
...
TYPED_TEST(GenericRecursiveAVLTreeTest, BasicInsertionAndSearch) {
    AVLTree<TypeParam> testTree;
    auto               testData = generateDataForSimpleTypedTests<TypeParam>();
    
    EXPECT_TRUE(testTree.empty());

    EXPECT_TRUE(testTree.insertRecursive(testData[0]));
    EXPECT_TRUE(testTree.insertRecursive(testData[1]));
    EXPECT_TRUE(testTree.insertRecursive(testData[2]));
    EXPECT_EQ(testTree.size(), 3);

    EXPECT_TRUE(testTree.searchRecursive(testData[0]));
    EXPECT_TRUE(testTree.searchRecursive(testData[1]));
    EXPECT_TRUE(testTree.searchRecursive(testData[2]));
    EXPECT_FALSE(testTree.searchRecursive(testData[3]));
    testTree.clear();
    return;
}


TYPED_TEST(GenericRecursiveAVLTreeTest, SingleRotationsLeftLeft) {
    AVLTree<TypeParam> testTree;
    auto               testData = generateDataForSimpleTypedTests<TypeParam>();
...
TYPED_TEST(GenericRecursiveAVLTreeTest, SingleRotationsRightRight) {
    AVLTree<TypeParam> testTree;
    auto               testData = generateDataForSimpleTypedTests<TypeParam>();

    EXPECT_TRUE(testTree.insertRecursive(testData[1]));
    EXPECT_TRUE(testTree.insertRecursive(testData[2]));
    EXPECT_TRUE(testTree.insertRecursive(testData[3]));
    ...
    EXPECT_TRUE(testTree.searchRecursive(testData[1]));
    EXPECT_TRUE(testTree.searchRecursive(testData[2]));
    EXPECT_TRUE(testTree.searchRecursive(testData[3]));
    EXPECT_FALSE(testTree.searchRecursive(testData[0]));
    testTree.clear();
    return;
}

TYPED_TEST(GenericRecursiveAVLTreeTest, DoubleRotationsLeftRight) {
...
TYPED_TEST(GenericRecursiveAVLTreeTest, DoubleRotationsRightLeft) {
...
TYPED_TEST(GenericRecursiveAVLTreeTest, DeletionRebalancing) {
    AVLTree<TypeParam> tree;
    auto               testData = generateDataForRebalanceTest<TypeParam>();

    for (auto& v : testData) {
        EXPECT_TRUE(tree.insertRecursive(v));
    }

    // Remove leaf
    EXPECT_TRUE(tree.removeRecursive(testData[3]));
    EXPECT_FALSE(tree.searchRecursive(testData[3]));
    EXPECT_TRUE(tree.isBalanced());

    // Remove node with two children (often requires finding successor)
    EXPECT_TRUE(tree.removeRecursive(testData[1]));
    EXPECT_TRUE(tree.isBalanced());
...
TYPED_TEST(GenericRecursiveAVLTreeTest, StochasticStressTest) {
    ...
    for (uint32_t i = 0; i < GenericRecursiveAVLTreeTest<TypeParam>::gk_stest_total_ops; ++i) {
        ...
        switch(op) {
            ...
            if(tree.searchRecursive(val) == true) {
                continue;
            }

            status = tree.insertRecursive(val);
